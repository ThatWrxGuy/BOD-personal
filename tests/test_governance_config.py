...
class TestGovernanceConfig:
    ...
    def test_council_members_from_config(self):
        ...
        for domain_name, config in DOMAIN_CONFIG.items():
            ...
            assert member.vote_weight == config["weight"]
    ...
    def test_risk_thresholds_from_config(self):
        ...
        for domain_name, config in DOMAIN_CONFIG.items():
            domain_key = domain_name.value
            expected_thresholds = config["risk_thresholds"]
            ...
            assert domain_threshold.max_risk_score == expected_thresholds[0]
            assert domain_threshold.escalation_threshold == expected_thresholds[1]
            assert domain_threshold.requires_approval_above == expected_thresholds[2]
    ...
    def test_domain_priorities_from_config(self):
        ...
        for domain_name, config in DOMAIN_CONFIG.items():
            domain_key = domain_name.value
            expected_priority = config["priority"]
            ...
            assert domain_priority == expected_priority
